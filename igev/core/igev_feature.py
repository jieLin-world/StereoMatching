...
from core.extractor_dnlr import Channel_Attention_Transformer_Extractor, ResidualBlock
...
class hourglass(nn.Module):
    def __init__(self, in_channels):
        ...
        self.feature_att_32 = FeatureAtt(in_channels*6, 128)
        self.feature_att_up_16 = FeatureAtt(in_channels*4, 128)
        self.feature_att_up_8 = FeatureAtt(in_channels*2, 128)

    def forward(self, x, features):
        conv1 = self.conv1(x)
        conv1 = self.feature_att_8(conv1, features[0])

        conv2 = self.conv2(conv1)
        conv2 = self.feature_att_16(conv2, features[1])

        conv3 = self.conv3(conv2)
        conv3 = self.feature_att_32(conv3, features[2])

        conv3_up = self.conv3_up(conv3)
        conv2 = torch.cat((conv3_up, conv2), dim=1)
        conv2 = self.agg_0(conv2)
        conv2 = self.feature_att_up_16(conv2, features[1])

        conv2_up = self.conv2_up(conv2)
        conv1 = torch.cat((conv2_up, conv1), dim=1)
        conv1 = self.agg_1(conv1)
        conv1 = self.feature_att_up_8(conv1, features[0])

        conv = self.conv1_up(conv1)

        return conv

class IGEV(nn.Module):
    def __init__(self, args):
        ...
        self.change_channel = nn.Conv2d(304, 96, kernel_size=1, padding=0, stride=1)
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, test_mode=False):
        ...
        print('use the feature_igev')
        image1 = (2 * (image1 / 255.0) - 1.0).contiguous()
        image2 = (2 * (image2 / 255.0) - 1.0).contiguous()
        with autocast(enabled=self.args.mixed_precision):
            outputs04, outputs08, outputs16, x = self.extractor(torch.cat((image1, image2), dim=0))
            #*cnet_list, x = self.extractor(torch.cat((image1, image2), dim=0))
            print(outputs04[0])
            #print(outputs08.shape)
            #print(outputs16.shape)
            ...
            stem_4y = self.stem_4(stem_2y)#[B, 48, H/4, W/4]

            features_left_init = torch.cat((fmap1, stem_4x), 1)#[B, 304, H/4, W/4]
            features_right_init = torch.cat((fmap2, stem_4y), 1)
            features_left = self.change_channel(features_left_init)#[B, 96, H/4, W/4]
            features_right = self.change_channel(features_right_init)

            match_left = self.desc(self.conv(features_left))#[B, 96, H/4, W/4]
            match_right = self.desc(self.conv(features_right))

            gwc_volume = build_gwc_volume(match_left, match_right, self.args.max_disp//4, 8)#[B, 8, 48(maxdip192//4), H/4, W/4]
            gwc_volume = self.corr_stem(gwc_volume)#[B, 8, 48, H/4, W/4],conv
            gwc_volume = self.corr_feature_att(gwc_volume, features_left)#[B, 8, 48, H/4, W/4]
            geo_encoding_volume = self.cost_agg(gwc_volume, [x[0] for x in cnet_list])
            ...
            if not test_mode:
                xspx = self.spx_4(features_left)#[B, 24, H/4, W/4]
        ...
        for itr in range(iters):
            ...
            with autocast(enabled=self.args.mixed_precision):
                if self.args.n_gru_layers == 3 and self.args.slow_fast_gru: # Update low-res ConvGRU
                    net_h = self.update_block(net_list, net_ext, iter16=True, iter08=False, iter04=False, update=False)
                if self.args.n_gru_layers >= 2 and self.args.slow_fast_gru:# Update low-res ConvGRU and mid-res ConvGRU
                    net_h = self.update_block(net_list, net_ext, iter16=self.args.n_gru_layers==3, iter08=True, iter04=False, update=False)
                net_list, mask_feat_4, delta_disp = self.update_block(net_h, net_ext, geo_feat, disp, iter16=self.args.n_gru_layers==3, iter08=self.args.n_gru_layers>=2)

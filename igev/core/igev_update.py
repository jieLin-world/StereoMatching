...
class IGEVStereo(nn.Module):
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, test_mode=False):
        ...
        with autocast(enabled=self.args.mixed_precision):
            ...
            gwc_volume = build_gwc_volume(match_left, match_right, self.args.max_disp//4, 8)#[B, 8, 48(192/4), H/4, W/4]
            gwc_volume = self.corr_stem(gwc_volume)#[B, 8, 48(192/4), H/4, W/4]
            gwc_volume = self.corr_feature_att(gwc_volume, features_left[0])#[B, 8, 48(192/4), H/4, W/4]  
            geo_encoding_volume = self.cost_agg(gwc_volume, features_left)#[B, 8, 48(192/4), H/4, W/4]
            
            
            # Init disp from geometry encoding volume
            prob = F.softmax(self.classifier(geo_encoding_volume).squeeze(1), dim=1)#[B, 48, H/4, W/4]
            init_disp = disparity_regression(prob, self.args.max_disp//4)#[B, 1, H/4, W/4]
            
            del prob, gwc_volume

            if not test_mode:
                xspx = self.spx_4(features_left[0])#[B, 24, H/4, W/4]
                xspx = self.spx_2(xspx, stem_2x)#[B, 64, H/2, W/2]
                spx_pred = self.spx(xspx)#[B, 9, H, W]
                spx_pred = F.softmax(spx_pred, 1)#[B, 9, H, W]


            cnet_list = self.cnet(image1, num_layers=self.args.n_gru_layers)#context features, a tuple:
            #([[B, 128, H/4, W/4],[B, 128, H/4, W/4]], [[B, 128, H/8, W/8],[B, 128, H/8, W/8]], [[B, 128, H/16, W/16],[B, 128, H/16, W/16]])
            net_list = [torch.tanh(x[0]) for x in cnet_list]#initial hidden, a list:
            #[[B, 128, H/4, W/4], [B, 128, H/8, W/8], [B, 128, H/16, W/16]]
            inp_list = [torch.relu(x[1]) for x in cnet_list]#context information, a list:
            #[[B, 128, H/4, W/4], [B, 128, H/8, W/8], [B, 128, H/16, W/16]]
            
            inp_list = [list(conv(i).split(split_size=conv.out_channels//3, dim=1)) for i,conv in zip(inp_list, self.context_zqr_convs)]
            #[[[B, 128, H/4, W/4],[B, 128, H/4, W/4],[B, 128, H/4, W/4]], [[B, 128, H/8, W/8],[B, 128, H/8, W/8],[B, 128, H/8, W/8]], [[B, 128, H/16, W/16],[B, 128, H/16, W/16],[B, 128, H/16, W/16]]]
        ...
        coords = torch.arange(w).float().to(match_left.device).reshape(1,1,w,1).repeat(b, h, 1, 1)#[B, H/4, W/4, 1]

        disp = init_disp
        disp_preds = []

        # GRUs iterations to update disparity
        for itr in range(iters):#iters == 12
            disp = disp.detach()
            geo_feat = geo_fn(disp, coords)#[B, 162, H/4, W/4]
            with autocast(enabled=self.args.mixed_precision):
                ...
                #net_list: [[B, 128, H/4, W/4], [B, 128, H/8, W/8], [B, 128, H/16, W/16]]
                #mask_feat_4: [B, 32, H/4, W/4]
                #delta_disp: [B, 1, H/4, W/4]

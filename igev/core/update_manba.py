...
class VMRNN(nn.Module):
    def __init__(self, hidden_dim, input_dim, input_resolution, kernel_size, num_heads, window_size, depth = 2,
        ...
        self.conv = nn.Conv2d(hidden_dim + input_dim, hidden_dim, kernel_size, padding=kernel_size // 2)

    def forward(self, c, h, c1, c2, *x_list):
        ...
        cell = torch.tanh(self.VSBs(hx)+c1)
        Ft = torch.sigmoid(self.VSBs(hx)+c2)
        #o_t = outputs[-1]
...
class BasicMultiUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dims=[]):#hidden_dims==[128]*3
        super().__init__()
        self.args = args

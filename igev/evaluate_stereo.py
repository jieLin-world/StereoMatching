...
from core.igev_SGE import IGEV, autocast
from core import stereo_datasets as datasets
...
def validate_eth3d(model, iters=32, mixed_prec=False):
    ...
    f = open('test_eth3d.txt', 'a')
    f.write("Validation ETH3D: EPE %f, D1 %f\n" % (epe, d1))
...
def validate_kitti(model, iters=32, mixed_prec=False):
    ...
    f = open('test_kitti.txt', 'a')
    f.write("Validation KITTI: EPE %f, D1 %f\n" % (epe, d1))
...
def validate_sceneflow(model, iters=32, mixed_prec=False):
    ...
    model.eval()
    val_dataset = datasets.SceneFlow(dstype='frames_finalpass', is_test=True)
    ...
    f = open('test_sceneflow024.txt', 'a')
    f.write("Validation Scene Flow: EPE %f, D1 %f\n" % (epe, d1))
...
def validate_middlebury(model, iters=32, split='F', mixed_prec=False):
    ...
    f.write("Validation Middlebury: EPE %f, D1 %f\n" % (epe, d1))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--restore_ckpt', help="restore checkpoint", default='/mnt/cephfs/dataset/stereo_matching/output/igev/checkpoints/sceneflow/igev-stereo.pth')
    ...
    model = torch.nn.DataParallel(IGEV(args), device_ids=[0,1,2,3,4,5,6,7])
